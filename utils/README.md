...
Questo script rappresenta uno strumento versatile per interagire in vari modi con un bucket esterno al cluster. In particolare, utilizza **Boto3**, una libreria Python che consente di comunicare con **object storage compatibili con il protocollo S3**, tecnologia proprietaria di AWS. Grazie a questa compatibilità, lo script può interfacciarsi anche con bucket MINIO, una soluzione open source per l'object storage che supporta lo stesso protocollo.

Di seguito vengono spiegati i metodi della classe BucketHandler utilizzati per effettuare una connessione al bucket minio(**__init__**), caricare il dataset MNIST (**upload_file()**) e scaricare il modello ottenuto dal training distribuito (**download_file()**):
